...
func (c *Crafty) StopMcServer(port int) error {
	...
	for _, server := range serverList.Data {
		if server.Port == port {
			found = true
			log.Printf("[DEBUG] Attempting to stop server ID=%s with port=%d", server.ServerID, server.Port)
			if err := c.sendStopServerRequest(server, bearer); err != nil {
				// Log and continue; don't fail the whole loop
				log.Printf("[WARN] Failed to stop server ID=%s: %v", server.ServerID, err)
			}
		}
	}
...
func (c *Crafty) sendStopServerRequest(server Server, bearer string) error {
	...
	// Log which server is being targeted
	log.Printf("[DEBUG] Sending stop request to server ID=%s on port=%d at URL=%s\n", server.ServerID, server.Port, stopServerURL)
